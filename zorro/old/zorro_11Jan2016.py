...
# between images
    ...
    def xcorr2_unblur( self, doseFilter = False, restoreNoise = True, minShift = 2.0, terminationThres = 0.1, maxIteration=10, verbose=False   ):
        """
        ...
        if bool( doseFilter ):
            # We have to compute the dose per frame in e/A^2
...
#        N = np.asarray( self.images.shape )[0] - 1
        ...
        if hasattr( triIndices, "__array__" ): # np.array
            ...
            if( self.triMode == 'first' ):
                ...
                N2 = np.int(N/2) # future division
                triIndices = np.zeros( [N,N] )
                for K in xrange(0,N2):
                    triIndices[K,N2+K] = True
        ...
        for I in rowIndices:
            ...
            for J in columnIndices:
                ...
                DenomBase = ne.evaluate( "real(tempComplex)*normConst2- real( Corr_baseMask * (Corr_baseMask / maskProduct) )" )
        ...
        del baseImageFFT_block, maskProduct, Corr_templateMask, normConst2
        ...
        return shiftsTriMat, corrTriMat, peaksigTriMat
...
#                    bounds=bounds, tol=pix_tol  )
    ...
    def alignImageStack( self ):
        ...
        """
        self.t[8] = time.time() 
        [shiftsTriMat1st, corrTriMat1st, peaksigTriMat1st] = self.xcorrnm2_tri()
        self.t[9] = time.time() 
        ...
        self.t[11] = time.time()
        if self.triMode == 'first':
            self.translations = -shiftsTriMat1st[0,:]
            ...
            self.shiftsSolver( shiftsTriMat1st, corrTriMat1st, peaksigTriMat1st )
            ...
            print( "Second round of equation pruning not applied" )
