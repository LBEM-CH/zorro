...
# between images
    ...
    def __init__( self ):
        ...
        # Dose filt param = [dosePerFrame, critDoseA, critDoseB, critDoseC, cutoffOrder]
        self.doseFiltParam = [None, 0.24499, -1.6649, 2.8141, 16]
...
#        N = np.asarray( self.images.shape )[0] - 1
        ...
        # if bool( self.doDoseFilter ):
            # Copy over the FFT'd images to the FFTsquare variable so we can have access to them later as a
            # seperate data set?  Or should this be done after shifting?
            #self.__baseSquaredFFT = self.__baseImageFFT.copy()
    ...
    def locatePeak( self, I, J ):
        ...
        if self.subPixReg > 1.0: # Subpixel peak estimation by Fourier interpolation
            ...
            if Csub.shape[0] == 2*self.__subR and Csub.shape[1] == 2*self.__subR:
                ...
                self.__CpadFFT.real = np.pad( np.fft.fftshift(self.__CsubFFT.real), ((self.subPixReg-1)*self.__subR,), mode='constant', constant_values=(0.0,)  )
                self.__CpadFFT.imag = np.pad( np.fft.fftshift(self.__CsubFFT.imag), ((self.subPixReg-1)*self.__subR,), mode='constant', constant_values=(0.0,)  )
...
#                    bounds=bounds, tol=pix_tol  )
    ...
    def alignImageStack( self ):
        ...
        # Hm, how to use the forward transformed data here without using Fourier wedge shifting, which really
        # seems to perform poorly given the STEM data.
        if bool(self.doDoseFilter):
...
#                # Normalize and reduce to float32
    ...
    def doseFilterNEW( self ):
        ...
        """
        # TODO: faster, better integrated as an intermediate step.  Unfortunately it's a pain to slot in 
        # smoothly with all the other options.
        pass
    ...
    def doseFilter( self ):
        ...
        dosePerFrame by default is estimated from the data. Currently we have no gain factor, so we assume 
        the input numbers are in electrons.  Optionally one may provide the dosePerFrame, which becomes 
        helpful on a thicker specimen, or one that has been pre-processed (i.e. binned from 8k to 4k).  Paramaters
        are set as follows:
        
            self.doseFiltParam = [dosePerFrame, critDoseA, critDoseB, critDoseC, cutoffOrder]
        ...
        FFT2, _ = util.pyFFTWPlanner( doseFilter, wisdomFile=os.path.join( self.cachePath, "fftw_wisdom.pkl" ) , 
                                     effort = self.fftw_effort, n_threads=self.n_threads, doReverse=False )
    ...
    def saveData( self ):
        ...
        if bool(self.doDoseFilter): # This will be in the same place as sum
            if not 'filt' in self.files or self.files['filt'] is None: # Default filename for filtered sum
    ...
    def loadConfig( self, configNameIn = None, loadData=False ):
        ...
        try: self.doseFiltParam = json.loads( config.get( 'registration', 'doseFiltParam' ) ) # This one stays a list
        except: pass
    ...
    def saveConfig( self, configNameIn=None ):
        ...
        if bool( self.doDoseFilter ):
            config.set( 'registration', 'doseFiltParam', json.dumps( self.doseFiltParam ) )
    ...
    def plot( self, title = "" ):
        ...
        #####      
        print( "TODO: CHECK IF WINDOWS NEEDS PROTECTION FROM MULTIPROCESSING MODULE" )
        if os.name != 'nt':
