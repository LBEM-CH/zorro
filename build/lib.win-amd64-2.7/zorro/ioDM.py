...
This is designed to be a fast DM4 file reader to strip the data out of the large
movie-mode files generated by K2 detectors along with important tags.  Due to 
the emphasis on speed it's not a general file parser with dynamic allocation and such, 
so if Gatan changes the format a lot it will break the script.
...
Can examine file structure by setting verbose = True, if you need to add some more 
tags to the dictionary generated by the parseTag function, for example
...
class DMImage(object):
    ...
    Because of the general format we can also use this for MRC and other formats.
    """
    def __init__( self ):
...
#        self.imageSum = None
#        self.FFTimage = None # Magnitude only
#        self.polar = None
#        self.FFTpolar = None # Magnitude only
        
        pass
    
#    def sum( self, axis=0 ):
#        print( "imageData.shape: " + str(self.imageData.shape) )
#        if self.imageData.ndim == 2:
#            self.imageSum = self.imageData
#        else:    
#            self.imageSum = np.sum( self.imageData, axis=axis )
#        
#    def FFT2( self, module = 'numpy' ):
#        """
#        Numpy Fourier transform
#        """
#        print( "DMImage computing FFT2" )
#        if self.imageSum is None:
#            self.sum()
#        if module == 'numpy':
#            self.FFTimage = np.abs( np.fft.fftshift( np.fft.fft2( self.imageSum ) ) )
#        else:
#            print( "DMImage: unknown module used for FFT" )
        
#    def polarFFT2( self, module = 'numpy' ):
#        """
#        Polar Fourier transform using numpy (slowish)
#        """
#        print( "DMImage computing polarFFT2" )
#        if self.FFTimage is None:
#            self.FFT2( module = module )
#        self.FFTpolar = util.img2polar( self.FFTimage )
        
class DM4Import(object):
    ...
    def parseTag( self, parent ):
        ...
        if self.verbose: print( "Found tag: " + parent + tag_name + " with " + str(tag_ninfo) + " elements" )
...
#t0 = time()
#filename = "C:/temp/test1042015.dm4"
#test = DM4Import( filename, verbose=False )
#t1 = time()
#print( "Import time (s): " + str(t1-t0) )
#
## See
#print test.im[1].imageInfo
#from plotting import ims
#ims( test.im[1].imageData[0,:,:] )
